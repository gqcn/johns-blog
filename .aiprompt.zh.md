...
#### Specs（规范，系统的事实来源）
specs/目录是整个项目的"行为事实来源"（source of truth），描述系统当前的行为方式。

#### Changes（变更，每个特性独立管理）
每一次特性开发或变更都对应changes/下的一个独立文件夹，包含该变更所需的全部上下文信息

#### Delta Specs（增量规范）
每次变更专属的行为变更清单，只描述本次变更对系统行为的增量影响，而非重写整份规范。
...
#### 第一步：探索需求
在开始正式规划之前，先用/opsx:explore澄清技术选型和边界

#### 第二步：提案与规划
需求清晰后，可以手动使用/opsx:propose一步生成完整的变更规划工件：proposal.md、specs/<domain>/spec.md、design.md、tasks.md
...
#### 第五步：归档
验证通过后，运行/opsx:archive完成整个变更周期

当AI完成开发和验证后，我进行了人工验证，发现仍然存在一些问题，我会通过自然语言的提示词信息描述告诉AI让其完成修复和验证。但这些提示词信息仍然存在于上下文中，我期望AI能将我提出的问题后，先整理修复任何和验证任务存储到文件中，比如tasks.md中，再执行修复和验证工作，以便完成修复工作和验证工作的闭环管理。
为了实现这个目的，我需要你创建一个skill来实现这个功能，当我对当前项目提出了一些bug或者改进点，并且该项目使用了openspec工具，那么你需要将这些问题进行整理分析，形成一个清晰的任务清单，并且将这个任务清单以工件的形式存储到当前变更文件夹下的tasks.md文件中。随后你需要按照这个任务清单来逐步执行修复和验证工作，直到所有任务都完成为止。
