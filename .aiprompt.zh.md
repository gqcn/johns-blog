...
背景：
1. 我需要编写一篇文章，介绍使用在AI模型开发训练推理业务场景下的CPU亲和性与NUMA亲和性调度。文章生成到该目录下: /Users/john/Workspace/github/gqcn/johns-blog/docs/docs/1000-AI技术/200-AI基础架构/700-算力调度/
2. 编写文章时，可以参考以下节点上的GPU拓扑信息：
```
$ nvidia-smi topo -m
...
要求：
1. 简要介绍CPU处理器的NUMA架构。
2. 简要介绍CPU亲和性以及NUMA亲和性对使用GPU进行AI模型开发训练推理的影响。
3. 通过给定的GPU拓扑示例介绍CPU亲和性和NUMA亲和性的具体含义。
4. 通过使用docker来使用CPU亲和性和NUMA亲和性调度的示例介绍。
5. 通过使用Kubernetes来使用CPU亲和性和NUMA亲和性调度的的技术方案及示例介绍。



按照以下要求改进文章内容：
1. 在【### 连接类型说明】栏目中，需要审查表格中【典型带宽】列下的数据，需要保证提供不同链接类型下带宽数据的准确性。比如PIX类型，确定最大带宽只有32GB/s? 理论上PCIE类型的GPU卡，那么应该会有2个GPU卡使用NVLINK Bridge互联，带宽会更高一些。请审查并修正该表格中【典型带宽】列下的数据。
2. 在【### 示例1：单GPU训练任务】栏目下，由于是单卡使用，指定--cpuset-cpus或者--cpuset-mems参数似乎毫无意义？
3. 在【### 示例2：多GPU训练任务（同NUMA节点）】栏目的示例中，我是否可以只指定--cpuset-cpus或者--cpuset-mems参数？如果两者不一致比如--cpuset-cpus指定的CPU处于NUMA NODE 1但是--cpuset-mems指定的却是NUMA NODE 0会有怎么样的docker表现行为或者性能表现行为？
4. 文中涉及到使用8卡GPU的示例统一改为5卡。