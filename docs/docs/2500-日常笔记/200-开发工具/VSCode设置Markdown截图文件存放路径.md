...
1. 打开`Settings`
    ![VSCode打开设置界面](assets/VSCode设置截图文件存放路径/image.webp)
...
3. 新增项:
    ...
    - Key为: `**/*.MD`, Value为目标路径：`assets/${documentBaseName}/${fileName}`
    ![Markdown Copy Files Destination配置项](assets/VSCode设置截图文件存放路径/image-1.webp)

4. 如果以上方式仍旧无效，那么可以尝试搜索`paste`，找到`Paste As: Enabled`选项，打勾：

    ![Paste As Enabled选项配置](assets/VSCode设置截图文件存放路径/image-2.webp)