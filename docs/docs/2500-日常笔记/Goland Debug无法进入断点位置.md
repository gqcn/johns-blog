...
![Goland中设置断点的Hello World程序代码](<assets/Goland Debug无法进入断点位置/image.webp>)

启动`Debug`后，总是会跳转到`asm_arm64.s`的源码文件处：

![调试器跳转到asm_arm64.s汇编源码文件](<assets/Goland Debug无法进入断点位置/image-1.webp>)

`Goland`版本为当前最新版本`2024.3.5`：

![Goland版本号2024.3.5显示界面](<assets/Goland Debug无法进入断点位置/image-2.webp>)
...
![调试功能恢复正常，成功进入断点位置](<assets/Goland Debug无法进入断点位置/image-3.webp>)
