...
```
![Volcano调度器队列层级结构检查错误日志](assets/5000-Volcano层级队列配置引发的调度器系统性故障问题/image.webp)
或者
```text
...
```
![Volcano调度器root队列资源容量检查失败错误日志](assets/5000-Volcano层级队列配置引发的调度器系统性故障问题/image-1.webp)
