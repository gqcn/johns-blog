...
`AI`有一种倾向：**生成比需求更复杂的解决方案**。正如`Birgitta Böckeler`在《[The role of developer skills in agentic coding](https://martinfowler.com/articles/exploring-gen-ai/13-role-of-developer-skills.html)》中记录的，这种过度工程会带来长期的维护成本：
...
理解了`AI`的局限之后，人工`CodeReview`的核心价值就变得清晰了。`Kief Morris`在《[Humans and Agents in Software Engineering Loops](https://martinfowler.com/articles/exploring-gen-ai/humans-and-agents.html)》中提出了一个重要框架：人类在`AI`辅助开发中应处于"**on the loop**"（在循环之上）的位置——不是微观管理每一行代码，也不是完全脱离审查，而是**构建和管理产出质量的保证机制**。
...
`Birgitta Böckeler`的一个重要提醒：当团队大量依赖`AI`生成代码时，开发者需要**主动保持自身的技术能力**。如果审查者无法理解所审查的代码，审查就失去了价值。
