...
- **上下文重置（Context Reset）与压缩（Compaction）的区别**：压缩（`/compact`命令）是将对话早期内容就地摘要化，使同一个`Agent`在缩短后的历史上继续执行——可以释放空间，但因为未提供真正的"干净石板"，**无法消除上下文焦虑**。上下文重置则是彻底清空当前窗口，启动一个全新`Agent`会话，并通过精心设计的交接文件（`Handoff Artifact`）传递上一个`Agent`的任务状态与下一步骤。重置能从根本上消除上下文焦虑，代价是更高的编排复杂度与`token`开销。当模型在压缩后仍出现提前收尾行为时，上下文重置往往是唯一有效的解决方案。
