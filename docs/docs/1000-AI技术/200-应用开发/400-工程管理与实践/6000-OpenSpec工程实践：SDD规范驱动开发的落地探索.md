...
![OpenSpec工程实践：SDD规范驱动开发的落地探索](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-2.webp)
...
`OpenSpec`的核心概念只有四个（`specs`、`changes`、`delta specs`、`artifacts`），五分钟内即可理解并开始使用。

#### 易于定制扩展

`OpenSpec`的设计遵循"流动而非刚性"原则，可以根据项目需求灵活添加自定义技能，弥补其功能简单的短板。

#### 工具兼容性广

与几乎所有主流`AI`开发工具（`Claude Code`、`Cursor`、`GitHub Copilot`等）兼容，不受特定平台约束。


#### 满足大部分需求

对于中小规模的`AI`协同开发项目，`OpenSpec`的核心工作流 - 探索、提案、实现、归档 - 已能满足绝大部分场景需求，额外的测试和审查能力通过自定义技能补全。
...
> 这一阶段通常使用高复杂度、高模糊度、强推理，且上下文窗口较大的模型，例如`Claude Opus 4.6 1M`。
...
![OpenSpec工程实践：探索式对话](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-5.webp)
![OpenSpec工程实践：探索式对话示意](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-6.webp)
![OpenSpec工程实践：探索式对话示意](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image.webp)
...
![OpenSpec工程实践：生成正式变更提案](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-7.webp)

![OpenSpec工程实践：生成正式变更提案示意](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-8.webp)

#### 第三步：按任务清单逐条实现（`/opsx:apply`） - 手动或自动触发，建议手动

> 这一阶段通常使用高标准化、高重复、成本敏感的模型，例如`Claude Sonnet(1M)`或者高性价比的国内模型`GLM-5`。
...
![OpenSpec工程实践：按任务清单逐条实现](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-9.webp)
![OpenSpec工程实践：按任务清单逐条实现示意](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-10.webp)
...
![OpenSpec工程实践：处理用户反馈](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-12.webp)
![OpenSpec工程实践：处理用户反馈示意](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-13.webp)
...
![OpenSpec：归档变更（/opsx:archive）](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-3.webp)
![OpenSpec：归档变更后的增量规范同步](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-4.webp)
...
![OpenSpec工程实践：实践中的问题与改进方案](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/image-1.webp)
...
2. **利用`SubAgent`机制隔离上下文**：`Claude Code`以及部分主流的`AI`开发工具支持`SubAgent`机制，每个`SubAgent`拥有独立的上下文窗口。对于执行周期较长的任务（例如`e2e`测试），可以显式地告诉`Claude Code`根据任务的耦合性自行开启`SubAgent`来完成，避免主会话上下文过载。
...
![OpenSpec：迭代的熵治理](assets/6000-OpenSpec工程实践：SDD规范驱动开发的落地探索/968c49aac8bff7b9d828294f7cf4db9c.webp)
...
这三类文档（`OpenSpec`生成的规范产物、`CLAUDE.md`、自定义`Skill`）的语言选择并不相同，核心差异在于**受众是人还是`AI`**：

#### `OpenSpec`生成的规范产物（如`proposal.md`、`design.md`、`tasks.md`）
...
推荐使用中文（如果团队语言是中文）。`CLAUDE.md`的核心受众同样是项目团队成员，母语编写的规范约束可读性更高、维护更自然，这一协作效率上的收益足以弥补语义理解上的细微差异。

#### 自定义`Skill`（`SKILL.md`）内容

推荐使用英文。`Skill`文件的核心受众是`AI`，而非人类团队成员——由于主流大模型的训练语料以英文为主，英文编写的技能指令能获得更高的语义理解精准度和遵循一致性，同时也更易于在社区中共享和复用。技能中涉及的具体业务约束可通过引用`CLAUDE.md`来实现，无需在技能文件中重复定义业务上下文。
...
#### 早期迭代（建议前10个迭代）：保持较高的Review强度

项目初期是规范沉淀的关键窗口期，建议对`AI`生成的每一份代码变更保持较严格的人工`Review`。其目的不只是发现当次的问题，更重要的是：
- **识别`AI`的高频错误模式**：找出`AI`在该项目中反复犯错的地方（如特定框架用法、命名约定、错误处理方式等）
- **将规范约束固化**：将发现的问题转化为`CLAUDE.md`中的约束条目或自定义`Skill`，形成可复用的工程规范
- **建立质量基线**：通过充分的`Review`为项目建立可参照的代码质量标准，为后续降低`Review`频率打好基础

#### 中期迭代：引入工具辅助，减少人工干预

随着规范逐步完善，可通过工具手段提升`Review`效率、降低人工成本：
- **自动化规范检查**：基于`openspec-review`等`Skill`工具，让`AI`在提交前自动对照既定规范进行自查，主动识别违规点
- **完善E2E测试覆盖**：通过`openspec-e2e`等工具持续补充端到端测试用例，将用户可观察行为的验收自动化，形成有效的质量兜底机制

#### 长期迭代：降低频率，但保留关键节点的Review

随着项目的深入迭代，`AI`生成代码的质量会因规范约束的不断完善而逐步提升，人工`Review`的频率可以适当降低。但仍建议在以下场景保持必要的审查力度：
- **关键功能与复杂逻辑**：对于涉及核心业务、安全鉴权、数据一致性等高风险模块，不应完全依赖`AI`自查
- **新引入的技术方案或架构变更**：此类变更超出既有规范覆盖范围，需人工判断是否符合长期演进目标
- **发现质量退化信号时**：若某个阶段测试失败率上升或反馈问题集中，应及时恢复较高强度的人工`Review`

总体原则是：**前期投入规范建设是为了后期解放双手，但"解放"不等于"放弃"**。持续的审查意识是防止技术债务积累的最后一道防线。
