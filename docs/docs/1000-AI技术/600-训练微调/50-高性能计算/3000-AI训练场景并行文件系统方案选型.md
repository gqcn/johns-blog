...
`MeshFS`基于`XSEA`（`eXtreme Shared-Everything Architecture`，星海架构），采用四层纵向分层设计（来源：[XSKY MeshFS产品页](https://www.xsky.com/products/mesh-fs)）：
...
graph TD
    ...
    subgraph 接入层 Access Layer
        B1[Tier-0 本地NVMe缓存<br/>20~30μs读延迟<br/>瞬间吸收Checkpoint突发写]
        B2[协议融合网关<br/>POSIX / NFS / SMB<br/>S3 / HDFS]
    end

    subgraph 核心处理层 Core Processing Layer
        C1[分布式元数据引擎<br/>XScale 星拓<br/>支持亿级文件秒级检索]
        C2[Shared-Everything 调度<br/>任意节点访问全局数据<br/>线性扩展无热点]
    end

    subgraph 网络与持久层 Network & Persistence Layer
        D1[FlexPath™ 智能网络引擎<br/>RDMA / TCP 自适应<br/>一跳直读 Direct Read]
        D2[NVMe SSD 全闪存储池<br/>自适应EC纠删码<br/>100ms 故障切换]
    end

    subgraph 智能分层
        E1[温/冷数据自动下沉<br/>低成本对象存储<br/>访问时透明召回]
    end

    A1 & A2 & A3 -->|POSIX/S3/RDMA| B1
    A1 & A2 & A3 --> B2
    B1 & B2 --> C1
    C1 <--> C2
    C2 --> D1
...
| 分层 | 核心组件 | 关键能力 |
|------|---------|---------|
| 接入层 | `Tier-0`本地缓存 + 协议网关 | 近算缓存消除`I/O`墙；`POSIX/S3/HDFS`多协议统一命名空间 |
| 核心处理层 | `XScale`元数据引擎 + `Shared-Everything`调度 | 分布式元数据分片，亿级文件秒级检索；全局视角调度，线性扩展 |
| 网络与持久层 | `FlexPath™`网络引擎 + `NVMe`全闪池 | `RDMA/TCP`自适应，一跳直读减少`50%`读写开销；`100ms`故障切换 |
