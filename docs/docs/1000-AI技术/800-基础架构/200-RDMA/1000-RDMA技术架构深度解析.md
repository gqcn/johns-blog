...
![传统TCP/IP网络通信数据拷贝流程](assets/RDMA技术介绍/image.webp)
...
![TOE技术架构示意图](assets/RDMA技术介绍/image-1.webp)
...
![RDMA整体框架架构图](assets/RDMA技术介绍/image-2.webp)
...
![三种RDMA硬件架构对比：InfiniBand、iWARP、RoCE](assets/RDMA技术介绍/image-3.webp)
...
![InfiniBand历史发展时间线](assets/RDMA技术介绍/image-4.webp)
...
![InfiniBand各代速率演进：从SDR到NDR](assets/RDMA技术介绍/image-5.webp)
...
![InfiniBand子网架构及设备组成](assets/RDMA技术介绍/image-6.webp)

在一个子网中，必须有至少每个节点有一个`channel adapter`，并且有一个`subnet manager` 来管理`Link`。

![InfiniBand子网拓扑示例](assets/RDMA技术介绍/image-7.webp)
...
![InfiniBand Switch转发原理](assets/RDMA技术介绍/image-8.webp)
...
![InfiniBand协议栈分层结构](assets/RDMA技术介绍/image-9.webp)

对应着不同的层，数据包的封装如下，下面将对每一层的封装详细介绍：

![InfiniBand数据包封装结构](assets/RDMA技术介绍/image-10.webp)
...
![InfiniBand物理层不同Link速度规格](assets/RDMA技术介绍/image-11.webp)
...
![QDR速率计算示例：4x QDR达到64Gb/s](assets/RDMA技术介绍/image-23.webp)
...
![InfiniBand链路层Virtual Lanes和QoS机制](assets/RDMA技术介绍/image-12.webp)
...
![InfiniBand网络层GRH报头格式](assets/RDMA技术介绍/image-13.webp)
...
![InfiniBand传输层与TCP/IP协议栈对比](assets/RDMA技术介绍/image-14.webp)

按照连接和可靠两个标准，可以划分出下图四种不同的传输模式：

![InfiniBand四种传输模式：RC、UC、RD、UD](assets/RDMA技术介绍/image-16.webp)
...
![不同传输模式支持的操作类型对照表](assets/RDMA技术介绍/image-17.webp)

下面是传输层的 `Base Transport Header` 的结构，长度为 `12` 字节，指定了源 `QP` 和 目标 `QP`、操作、数据包序列号和分区。

![InfiniBand传输层BTH报头结构](assets/RDMA技术介绍/image-18.webp)
...
![RDMA扩展传输报头RETH结构](assets/RDMA技术介绍/image-19.webp)

下面是 `Datagram ETH`，面向与 `UD` 和 `RD` 类型的服务：

![Datagram扩展传输报头DETH结构](assets/RDMA技术介绍/image-20.webp)
...
![可靠数据报扩展传输报头RDETH结构](assets/RDMA技术介绍/image-21.webp)
...
![RoCE与InfiniBand协议栈对比](assets/RDMA技术介绍/image-22.webp)
