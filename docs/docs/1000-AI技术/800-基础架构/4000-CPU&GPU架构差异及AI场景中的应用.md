...
![CPU与GPU的架构设计差异](assets/7000-CPU与GPU的架构差异及AI场景中的应用/image.webp)
...
![流式多处理器示意图](assets/4000-CPU&GPU架构差异及AI场景中的应用/image.webp)

`Warp`：`warp`是调度和运行的基本单元，`warp`中所有`threads`并行的执行相同的指令。`warp`由`SM`的硬件`warp scheduler`负责调度，一个`SM`同一个时刻可以执行多个`warp`。

![Warp示意图](assets/4000-CPU&GPU架构差异及AI场景中的应用/image-1.webp)
...
![CPU*GPU存储架构差异](assets/4000-CPU&GPU架构差异及AI场景中的应用/image-2.webp)
