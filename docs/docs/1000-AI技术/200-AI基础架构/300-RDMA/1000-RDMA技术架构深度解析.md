...
![传统TCP/IP网络通信数据拷贝流程](assets/RDMA技术介绍/image.png)
...
![TOE技术架构示意图](assets/RDMA技术介绍/image-1.png)
...
![RDMA整体框架架构图](assets/RDMA技术介绍/image-2.png)
...
![三种RDMA硬件架构对比：InfiniBand、iWARP、RoCE](assets/RDMA技术介绍/image-3.png)
...
![InfiniBand历史发展时间线](assets/RDMA技术介绍/image-4.png)
...
![InfiniBand各代速率演进：从SDR到NDR](assets/RDMA技术介绍/image-5.png)
...
![InfiniBand子网架构及设备组成](assets/RDMA技术介绍/image-6.png)

在一个子网中，必须有至少每个节点有一个`channel adapter`，并且有一个`subnet manager` 来管理`Link`。

![InfiniBand子网拓扑示例](assets/RDMA技术介绍/image-7.png)
...
![InfiniBand Switch转发原理](assets/RDMA技术介绍/image-8.png)
...
![InfiniBand协议栈分层结构](assets/RDMA技术介绍/image-9.png)

对应着不同的层，数据包的封装如下，下面将对每一层的封装详细介绍：

![InfiniBand数据包封装结构](assets/RDMA技术介绍/image-10.png)
...
![InfiniBand物理层不同Link速度规格](assets/RDMA技术介绍/image-11.png)
...
![QDR速率计算示例：4x QDR达到64Gb/s](assets/RDMA技术介绍/image-23.png)
...
![InfiniBand链路层Virtual Lanes和QoS机制](assets/RDMA技术介绍/image-12.png)
...
![InfiniBand网络层GRH报头格式](assets/RDMA技术介绍/image-13.png)
...
![InfiniBand传输层与TCP/IP协议栈对比](assets/RDMA技术介绍/image-14.png)

按照连接和可靠两个标准，可以划分出下图四种不同的传输模式：

![InfiniBand四种传输模式：RC、UC、RD、UD](assets/RDMA技术介绍/image-16.png)
...
![不同传输模式支持的操作类型对照表](assets/RDMA技术介绍/image-17.png)

下面是传输层的 `Base Transport Header` 的结构，长度为 `12` 字节，指定了源 `QP` 和 目标 `QP`、操作、数据包序列号和分区。

![InfiniBand传输层BTH报头结构](assets/RDMA技术介绍/image-18.png)
...
![RDMA扩展传输报头RETH结构](assets/RDMA技术介绍/image-19.png)

下面是 `Datagram ETH`，面向与 `UD` 和 `RD` 类型的服务：

![Datagram扩展传输报头DETH结构](assets/RDMA技术介绍/image-20.png)
...
![可靠数据报扩展传输报头RDETH结构](assets/RDMA技术介绍/image-21.png)
...
![RoCE与InfiniBand协议栈对比](assets/RDMA技术介绍/image-22.png)
