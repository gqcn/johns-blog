...
**适用场景**：模型能够放入单个`GPU`显存的情况。

#### 1.3.2 模型并行（Model Parallelism）

当模型过大无法放入单个`GPU`时，需要将模型拆分到多个`GPU`：
- 模型的不同层分布在不同`GPU`上
- 数据在`GPU`间流动，依次经过各层
- 需要处理`GPU`间的激活值传递

**适用场景**：超大模型训练（如`GPT-3`、`LLaMA-70B`等）。
...
## 3. Kubeflow Trainer PyTorch并行计算支持
