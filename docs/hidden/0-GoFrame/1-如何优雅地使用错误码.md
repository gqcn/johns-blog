...
![Error Code](https://pic1.zhimg.com/80/v2-0d1656ac7e1f4a8b1e2c81c502a2e1c4_1440w.webp)
...
- **敏感信息隐藏**：错误码可以帮助隐藏敏感信息，避免将内部实现细节暴露给用户，例如数据库执行报错信息。

- **防止信息泄露**：直接返回异常栈信息可能会泄露系统结构，而错误码可以避免这一点。


## 整型与字符串错误码

错误码的类型常见有两种：整型与字符串型。在`Go`开发中，选择使用整型还是字符串作为错误码需要根据具体场景和需求来决定。以下是一些指导原则和最佳实践。
...
func HandleError(err error) {
    if code := gerror.Code(err); code == CodeInvalidInput {
        // 处理非法输入
    }
}
```
...
// 字符串错误码示例
const (
    ErrInvalidEmail = "user.invalid_email"
...
`pkg/errors`包装模式通过第三方库实现错误包装和堆栈跟踪，适用于需要错误追踪的场景。
...
> 在`GoFrame`的大仓设计中，`utility` 目录并非严格区分业务和非业务代码。我们推荐循序演进的项目架构设计，业务项目维护者可根据需要自行做区分。
...
在`GoFrame`项目中，我们可以将错误码分为不同的层级，可以通过不同的分类代码文件来维护。我们只需要定义和维护错误码即可，不建议定义具体的错误对象，原因如下：
- 错误码与错误对象时一对多关系，不同的错误对象可以带有不同的错误信息，例如：```gerror.NewCodef(1001, `user "%s" not found`, userName)```。
- 错误对象在创建时包含当前错误位置完整的错误堆栈，利于代码链式跟踪调试。
...
const (
    ...
    CodeValidateError = 2     // 验证错误
    CodeBusinessError = 3     // 业务错误
    CodeNotAuthorized = 401   // 未授权
...
const (
    ...
    CodePasswordInvalid  = 1002  // 密码错误
    CodeTokenExpired     = 1003  // 令牌过期
    CodeUserDisabled     = 1004  // 用户已禁用
    CodeUserExists       = 1005  // 用户已存在
)
```
...
// 订单模块错误码 (20xx)
const (
    CodeOrderNotFound    = 2001  // 订单不存在
    CodeOrderPaid        = 2002  // 订单已支付
    CodeOrderCancelled   = 2003  // 订单已取消
    CodePaymentFailed    = 2004  // 支付失败
)
```
### 3. 实际使用示例
