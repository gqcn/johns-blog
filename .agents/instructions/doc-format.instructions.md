...
如果编写的`markdown`文档是在`johns-blog`目录下或者其子级目录下，表明这是一篇博客文章，那么需要额外满足以下要求：
- 编写文档时，文档的内容结构需要参考同级目录的其他文档，确保结构一致性。
- 不要生成一级标题（# 标题），一级标题通常用于文档的主标题，已经由文档文件名提供。
- 顶部的frontmatter内容中的description字段需要简要概括文章内容，不能少于100字，不超过120字；keywords字段不能少于15个词。
- 顶部frontmatter中的description字段必须直接从文章讨论的痛点切入，概括需要解决的问题以及文章覆盖的关键内容；禁止以“本文……”或“文章……”等自述式表达开头。
